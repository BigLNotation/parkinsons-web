...
const RangeSlider: React.FC<RangeSliderProps> = ({ min, max, step, initialValue = min, name, id }) => {
    ...
    const currentColor = colorsHsv[Math.floor((percentage / 100) * (colorsHsv.length - 1))].toHexString();
    // background of the slider essentially sets up a linear gradient using tinygradient, then changes to white after the thumb
    const sliderBackground = `linear-gradient(to right, ${gradient.hsvAt(0).toHexString()} 0%, ${currentColor} ${percentage}%, white ${percentage}%, white 100%)`; 
