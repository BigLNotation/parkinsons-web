...
function SymptomCard({
  ...
  return (
    <button
      className={
        recentlyCompleted ? variants["highlighted"] : variants["normal"]
      }
      {...other}
    >
      <div className="flex gap-8 items-center justify-between ">
        ...
        <div className="w-[70px] h-[70px] flex items-center justify-center rounded-full bg-white">
          <img
            src={symptomIcons[title] || symptomIcons["Tremors"]}
            alt={`${title} icon`}
        ...
        <div className="w-[220px] flex flex-col gap-5 text-left">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-md font-normal text-gray-300 leading-none">
              {description}
            </p>
          </div>
          <p
            className={
              recentlyCompleted
                ? statusVariants["highlighted"]
                : statusVariants["normal"]
            }
          >
            {status}
          </p>
          <div>
            <Button href={`/form/${formId.$oid}`} as="a" variant="secondary">
              Track ➜
            </Button>
          </div>
        </div>
